...
|---|---|---|---|---|---|---|
| C1 | Per-resolution L1 normalization complicates the interpretation of $G=D_1-D_8$ | Valid. $G$ is a resolution contrast, not a general monotonicity theorem. The review also exposed a manuscript/code mismatch: epsilon was printed in the denominator, whereas code used it only as a zero-mass tolerance. | major | ACCEPT_TEXT + ACCEPT_ANALYSIS | text corrected; monotone supplement pending | Keep registered $G$ unchanged; add a separately named fixed-fine-normalization diagnostic. |
| C2 | Evidence is entirely synthetic | Valid and the main external-validity weakness. | major | ACCEPT_EXPERIMENT | TODO_EXPERIMENT | Semi-synthetic contexts must be frozen before model access and cannot be described as real causal validation. |
| C3 | Common trajectory metrics are not compared | Valid. Existing fixture validation shows selectivity but does not establish comparative necessity. | major | ACCEPT_ANALYSIS | TODO_ANALYSIS | Use the same analytic fixtures; report values and desiderata without outcome-driven metric selection. |
| C4 | No response-path figure | Valid and unusually costly for a response-shape paper. The frozen outline already required response and resolution figures, so this is an omitted planned deliverable rather than a new reviewer-driven hypothesis. | major presentation | ACCEPT_FIGURE | TODO_AUTHOR_CONFIRM | Use all three passing cells and a deterministic representative-selection rule. |
| C5 | Thresholds and binary Pass appear arbitrary | Partly valid. Pre-inference freezing addresses researcher degrees of freedom, not universal practical significance. A sensitivity map can show stability but cannot retroactively justify the original values. | medium | ACCEPT_ANALYSIS + SOFTEN_CLAIM | TODO_ANALYSIS | Preserve effect sizes and intervals as primary; keep Pass as the registered decision only. |
| C6 | Quantile robustness, WQL, and epsilon are underreported | Epsilon is a real reporting error and is corrected. WQL already exists in stored P1 arrays and should be reported. Quantile-wise response distances require new inference because quantile trajectories were not archived. Their interpretation also differs from a structural individual response. | mixed | ACCEPT_TEXT + ACCEPT_ANALYSIS + PARTIAL | epsilon done; WQL pending; quantile analysis deferred | Do not present differences between marginal forecast quantiles as individual causal effects. |
| C7 | TimesFM-3 patching may explain temporal distortion | Plausible analysis hypothesis. The official architecture uses 32-step patches, but a single-model association is not causal evidence. | medium/high | ACCEPT_ANALYSIS | deferred until P2 | A frozen resampling-by-resolution study is needed before making an architectural claim. |
